...
| 模块 | 职责 |
|---|---|
| `platform` | GitLab / GitHub：URL 解析、host 匹配 `[[platforms]]`、鉴权、拉 diff 与定位 SHA、发评论、查已有评论；并把**平台侧的仓库读取能力**抽象成 `RepoSource`（列文件树、按 sha 取单文件正文、代码搜索）连同一份 `capabilities()`，说明这个实例究竟支持哪几项 |
