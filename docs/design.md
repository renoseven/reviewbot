...
**工具全量注册，能力是 worktree 的属性**。worktree 报一份**它提供什么**（`Reach::provides() -> Abilities`，位集合），工具声明一份**它需要什么**（`Tool::needs()`），两边一减就是这次答不上来的部分（`Reach::unmet(needs)`）。目前是三位：有没有内容可读（纯 diff 输入且没有平台就没有）、答不答得了搜索（平台有没有代码搜索）、是不是完整检出（`requires_checkout` 的检查器要这一位）。它们决定的不是「这个工具在不在」，而是「这一次它答不答得上来」——名字每次都是同一套，由模型看着描述决定何时调哪个。

**做成匹配而不是逐个工具写规则**，是为了把两件纠缠的事拆开：「提不提供」是集合运算，可以脱离措辞单独测；「提供不了该怎么说」是另一张表。再加一位能力，改的是 `provides()` 一位加一条措辞，没有工具要回头改。**许可不是能力，不进这个集合**：`requires_build` 要的是 `[security].allow_build_tools`，那是运维授予的、启动即失败的东西，worktree 既不提供也不收回它，混进来会让「这次做不到」和「你没批准」共用一套话。
...
顺带三处跟着改：**「注册了却没被调用的检查器」要留痕的那条，数的是「本次答得上来的检查器」**——这次注定拒绝的那个，不算模型漏掉的；**分片上限预留的工具余量同理**，只会回一句拒绝的工具不占额度；**空 worktree 不缩短评审**——分片照跑、模型照调，缺能力只影响布局摘要与「开工前把文件放进 worktree」那一步。
...
**自描述**：`name`、`description`（给模型看：这工具做什么、何时该用、本次能不能用）、参数声明（JSON Schema 由它派生）、用途、可用轮次、以及本次答不答得上来（`unavailable`，由 `requires_checkout` 这类声明配上这次的 `Reach` 算出）。
...
- **切分**：**一个分片就是一个文件**，绝不把两个文件塞进同一个分片。单文件超过分片上限时才按 hunk 再切（一个 hunk 绝不腰斩，上限再小也是一 hunk 一片），此时同一文件的几个分片**各自一次对话**，但不是彼此无知的——交接见 [§7 分片交接](#分片交接单文件被切开时)。
  ...
  三项都在启动校验之后算得出来——那时这次 worktree 能答什么已成定局（[§6 可扩展](#可扩展)）。工具余量按「本次答得上来的检视类工具」预留：只会回一句拒绝的工具不往 `input` 里堆正文，不占额度。`[triage].max_chunk_tokens` 必填无默认（[§5](#数值项一律必填代码里没有兜底值)），可以比示例里的 `24000` 更大或更小，但不能超过硬顶。

  **工具余量是乘出来的，配置改一个数就可能把窗口吃光**：轮数乘每轮字节数增长很快，一个 128k 窗口的模型经不起几轮 64KB。所以余量算完还要看剩下多少——不够放下一份最小的 diff（连文件头、一个 hunk 和模型要逐字引回来的那几行都装不下）就**启动即失败**，错误里点名 `[review].max_tool_rounds` 和 `[review].max_tool_output_bytes` 这两个旋钮。不这么做的话，症状会是每个文件都以 `ChunkTooLarge` 收场，一个分片一次模型调用地烧过去。这道检查只在真有检视类 tool 答得上来时才有意义，所以它跟余量本身一样落在 `triage` 算分片上限的地方，不在启动校验里——那时还不知道这次的 worktree 长什么样。
...
**三项能力里只有搜索是可选的**，所以 `platform` 除了这几个方法还要给一份 `capabilities()`：这个实例到底支不支持代码搜索，支持的是正则还是只有关键词索引。平台答不了搜索时，run 自己开的 worktree 就答不了搜索，`search_code` 照样注册，但描述里标明本次不可用、真调了回一句「这次没人能答，这不是『搜不到』」，而不是回一个空结果（[§6 可扩展](#可扩展)）。
...
**它也不冒充完整检出。**「需要整个工程」是一项独立的能力（`requires_checkout`）：只看单个文件的检查器两种 worktree 上都答得上来，要整份工程的（版本历史、编译、跨文件分析）在 run 自己开的 worktree 上答不了，调它就拒。一个 `.c` 文件缺了项目头文件，编译型检查器会报一屏 missing include，比不跑更糟。

**worktree 一定存在，但可能是空的。** 纯 diff 输入且没有平台时它读不到任何代码，此时内容工具与检查器一个也答不上来。工具照样全套给模型，每一条的描述与拒绝都写明「这次读不到」并且「这跟仓库里有没有无关」（[§6 可扩展](#可扩展)）。

下表说的是**每格里各项能力答不答得上来**，不是有没有这个工具：工具清单每次都一样。

| 输入 | 有检出（`--worktree`） | 无检出 |
|---|---|---|
| MR/PR URL | worktree 是那份检出，先校验 HEAD == `head_sha`；四个内容工具都答得上来，检索是本地正则；`requires_checkout` 的检查器也答得上来 | worktree 是 run 自己开的一份，缺文件时按 `head_sha` 取回落盘；列目录与检索转发平台（平台答不了搜索，`search_code` 就拒）；`requires_checkout` 的检查器拒 |
| diff 文件 | 同上，只是 diff 里没有 sha 可对照，校验不了；检出 HEAD 记进摘要并参与 `run_id` | **worktree 是空的**，内容工具与检查器全拒，模型这一轮只有手上这份 diff |
