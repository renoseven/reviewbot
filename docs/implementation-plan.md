...
按 [设计](design.md) §12 的顺序推进：先立骨架（每层可编译、假实现能空跑），再把 diff → 报告 → 发帖这条真链路打通，最后往回路里加工具与仓库内容来源。测试取「最小」档：每个模块留少量单元测试保证回归，不铺 §11 的四十组用例，其余进 README 的 TODO。

本文是计划快照，跟 Cursor 的 plan 文件同源；设计以 [design.md](design.md) 为准，两处冲突时改这里。
