...
Two things worth knowing. Changing the config keeps the same run id: the same command against an edited config re-enters that run and drops the stages the edit reaches, rather than opening a new directory or being refused. Only `--run-id`, which can aim at a directory that records another input, is refused — and that failure does not print `next:`, because the command is what pointed at the wrong directory. And `--publish` is re-read from the command line every time: a run you first launched with `--publish` and then re-enter without it records "do not post" and posts nothing that time.
...
- `[log] level` (`error|warn|info|debug|trace`, default `info`) is the one config field left out of the fingerprint slices, so turning the log up does not invalidate a run's checkpoints. The fingerprint is three slices named after the earliest stage each can reach (`input` / `triage` / `review`); it is not part of the run id. It is the only way to set the level; there is no `-v`.
...
- **How many rounds the tool loop gets is not configured.** The diff takes what `[triage].max_chunk_tokens` asks for, and every token the model's window has left over buys rounds — one round may add as much as a whole chunk may be. A 1M-token window with a 24k chunk works out to 24 rounds of 80KB; a run with nothing to investigate gets one. It used to be a pair of settings multiplied into a window reservation, which nobody could reason about, and the failure people hit was a ceiling too low: files whose investigation stopped half way, on a run that had spent 4% of its budget. A round ceiling guards against a model that never concludes; what a run may spend is `[[provider]].budget_per_run`, checked before every call.
- After each round the model is told how many it has used and how many there are, and warned when the next one is the last. It is asked to ration those rounds, so it is shown the balance.
