//! Unified diff parsing, owned by `input`. One parser serves both a local
//! diff file and the text a platform's diff endpoint returns, and it tells
//! the accepted format from the refused one by content, never by file name.

use std::collections::BTreeSet;

use crate::domain::{DEV_NULL, FileChange, Hunk};
