...
mod tests {
    ...
    use crate::platform::{
        Capabilities, LineRange, Listing, PlatformError, Repo, RepoSource, SearchHit,
    };
    use crate::security::PathPolicy;
    use crate::tool::{Purpose, ToolError};
...
"#;
    ...
    fn registry(settings: &Settings, worktree: Worktree) -> Registry {
        Registry::assemble(
            settings,
            PathPolicy::for_settings(settings).expect("valid globs"),
            Arc::new(worktree),
    ...
    fn the_catalog_prints_the_widest_shape_as_available() {
        ...
        let listing = Registry::catalog(&settings).expect("catalog");
        for name in [
            "list_local_files",
            "suggest_local_read",
            "read_local_file",
