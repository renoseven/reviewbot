...
use super::file::{Config, LogLevel, Model, ParamKind, PlatformEntry, Provider};
...
/// for real (`Preamble::tokens`) and reserves the larger of the two.
///
...
impl Settings {
    ...
    pub fn fingerprint(&self) -> Result<Fingerprint, ConfigError> {
...
"#;
    ...
    fn log_level_does_not_change_the_config_fingerprint() {
        ...
        assert_eq!(
            fingerprint::fingerprint(&info, None, false).expect("serializable"),
            fingerprint::fingerprint(&trace, None, false).expect("serializable")
        );
    }
