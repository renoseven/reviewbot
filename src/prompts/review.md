...
`start_line` and `end_line` are the hang span and are what the report prints: `start_line` is required (first line of the pin); `end_line` is the last line of that same span and may be omitted when it is a single line. Do not put the evidence there. `body` states the problem only; `suggestion` states the fix only. Both are required. `severity_score` and `confidence_score` follow section 6 below. `evidence.lines` is the evidence and must contain at least one file line number this change touched, or the finding is discarded.
